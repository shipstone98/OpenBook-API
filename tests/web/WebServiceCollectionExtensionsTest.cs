...
public sealed class WebServiceCollectionExtensionsTest
{
    ...
    public void TestAddOpenBookWebClaims_Valid()
    {
        ...
        ServiceDescriptor claims =
            collection.First(s =>
                s.ServiceType.Equals(typeof (IClaimsService)));

        Assert.Equal(ServiceLifetime.Scoped, claims.Lifetime);

        ServiceDescriptor claimsTransformation =
            collection.First(s =>
                s.ServiceType.IsAssignableTo(typeof (IClaimsTransformation)));

        Assert.Equal(ServiceLifetime.Scoped, claimsTransformation.Lifetime);
    }
